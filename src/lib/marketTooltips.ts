...
export function getMarketTooltips(marketId: string) {
  ...
  return {
    ...
    cap:
      `The Ceiling is the upper boundary of the BSSZ corridor, set at Anchor plus 20%. It is wider than the Floor by design, reflecting the asymmetric nature of energy commodity price shocks — prices spike upward far more violently than they collapse. A participant who wants to buy at elevated prices may do so within the corridor. If the physical market sustains that move, the Anchor will follow over subsequent days and the Ceiling will shift upward accordingly. The asymmetry is not arbitrary — it mirrors the structural behaviour of European energy markets during geopolitical and supply-side shocks — here, for ${m.commodity} on the ${m.marketName}.`,
